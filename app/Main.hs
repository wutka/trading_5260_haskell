...
main = do
  ...
  let updatesFile = if length argv > 1 then head argv else "data/updates.txt"
  let scoringFile = if length argv > 2 then argv !! 1 else "data/scoring2.txt"
  let resourcesFile = if length argv > 3 then argv !! 2 else "data/resources_part2.csv"
  let turns = if length argv > 4 then read (argv !! 3) else 10
  let maxDepth = if length argv > 5 then read (argv !! 4) else 4
  let frontierSize = if length argv > 6 then read (argv !! 5) else 200
  let numSchedules = if length argv > 7 then read (argv !! 6) else 10
  let gamma = if length argv > 8 then read (argv !! 7) else 1.0
